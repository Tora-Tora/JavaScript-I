...
// Assuming the inventory is sorted by ID in ascending order we can go ahead and use the [id-1] as position in the array. However if it isn't sorted then we should really run a for loop through the array to match the ID number and return the position in the array to reference in the console.

let array_position_of_car_33;
for ( let i = 0; i < inventory.length; i++) {
  if (inventory[i].id == 33) { array_position_of_car_33 = i;}
}

console.log("Car 33 is a " + inventory[array_position_of_car_33].car_year + " " + inventory[array_position_of_car_33].car_make + " " + inventory[array_position_of_car_33].car_model + ".");
...
// using .map
carYears = inventory.map(x => x.car_year);
